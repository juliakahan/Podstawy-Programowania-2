...
string & VectoredList::operator [] (const size_t index) const
{
    if(index > capasity)
...
VectoredList VectoredList::operator+(const VectoredList &rhs) const {
    VectoredList sum_result;
    for (int i = 0; i < this->getSize(); ++i){
        sum_result.push_back(this->operator [](i));
    }
    for (int i = 0; i < rhs.getSize(); ++i){
        sum_result.push_back(rhs[i]);
    }
    return sum_result;
}

void VectoredList::assign(string &&v) {
